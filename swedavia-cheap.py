...
for i in range (0,50):
    ...
    price_parameters = {
            ...
            "token" : "fc796b73d53c1b96aae3d2a7af9cdb76",
            "currency" : "SEK"}
    
##### TRYING TO FIGURE OUT HOW TO GET PRICES ####
    # priceURL = urllib.request.urlopen("http://api.travelpayouts.com/v2/prices/latest?currency=usd&period_type=year&page=1&limit=30&show_to_affiliates=true&sorting=price&trip_class=0&token=fc796b73d53c1b96aae3d2a7af9cdb76", data= price_parameters)
    # pricesdata = json.load(priceURL)
    # print(type(pricesdata))
    # pp.pprint(pricesdata)
    # print(10*"---")
    

    pricesURL = requests.get("http://api.travelpayouts.com/v1/prices/cheap?origin="+ airport + "&destination=" + destinationIATA + "&depart_date=" + today +" &return_date=2018-06-01&token=fc796b73d53c1b96aae3d2a7af9cdb76", data= price_parameters)
    ...
    row = dataID + ',' + flightID +','+ destinationEnglish+','+ destinationIATA+','+ continent+','+countryCode +','+ str(departureTime) +','+ flightStatus +','+ "str(pricesdata)" +','+ str(distance) +','+ str(emissions) +','+ str(taxCuts) +'\n'
    doc.write(row)
...
#     for i in range(len(flights)):
#         if(timenow > departureTimeParsed and "STHLM" in ["Flight Status"] ) :
...
#     # winsound.Beep(freq, duration)   
#     print(20*"===")
#     threading.Timer(1000, CheckStatus).start()
